...
for(nnn in 1:nrow(combos))
{
  ...
  filename<-paste("AIME dim 3 nx", nx, 'ny', ny, 'kx', kx, 'ky', ky, 'links', links, 'sn', sn, 'N', N,"rho",rho, ".bin")
  ...
  for(mmm in 1:n.simu)
  {
	...
	b<-aime(data.in=X, data.out=Y, in.layers=2, out.layers=2, max.epochs=50,importance.permutations=1, ncomp=3)
