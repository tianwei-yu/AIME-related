...
library(mvtnorm)
  ...
  filename<-paste("iCluster dim 3 nx", nx, 'ny', ny, 'kx', kx, 'ky', ky, 'links', links, 'sn', sn, 'N', N,"rho",rho, ".bin")
  ...
  if(length(is.there) == 0)
  {
	  ...
	  for(mmm in 1:n.simu)
	  {
		...
		data <- list(mir=(X), gene=(Y))
		results<-iCluster2(data, k=4)

		embeded<-results$expZ
		clust<-results$clusters
		w<-results$W[1:nx,]
		icluster.imp1<-apply(abs(w),1,max)
		icluster.imp2<-apply(w^2,1,mean)
		icluster.imp3<-apply(abs(w),1,mean)
		
		rec[[mmm]]<-cbind(icluster.imp1, icluster.imp2, icluster.imp3)
